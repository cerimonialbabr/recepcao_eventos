...
async function carregar(){
  try{
    const [convidados,mesasOriginais]=await Promise.all([API.convidados(),API.mesas()]);
    const mesas=[...mesasOriginais].sort((a,b)=>Util.compararMesas(a.mesa,b.mesa));
    ...
    const ultimos=[...presentes].sort((a,b)=>String(b.horaCheckin).localeCompare(String(a.horaCheckin),"pt-BR")).slice(0,15);
    document.getElementById("ultimosCheckins").innerHTML=ultimos.length
      ?ultimos.map((p)=>`<article><div><strong>${Util.escapar(p.nomeExibicao)}</strong>
       <span>Mesa ${Util.escapar(Util.mesaExibicao(p))}</span></div><time>${Util.escapar(p.horaCheckin)}</time></article>`).join("")
      :"<p>Nenhum check-in realizado.</p>";
  }catch(erro){document.getElementById("ultimaAtualizacao").textContent=`Erro: ${erro.message}`;}
}
