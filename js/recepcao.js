...
function validarApi(){
  ...
  if(
      ...
      throw new Error(
          "Informe a URL do Web App em js/config.js."
      );
...
function cartaoGrupo(grupo){
  ...
  const presentes=grupo.membros.filter(
      p=>p.checkin
  ).length;
  ...
  botao.innerHTML=`
      ...
      <div class="cartao-grupo-topo">
          ...
          <div>
              ...
              <span class="etiqueta">
                  ${Util.escapar(grupo.grupo)}
              </span>

              <h3>
                  ${Util.escapar(principal.nomeExibicao)}
              </h3>
          ...
          <span class="status ${presentes?"presente":"pendente"}">
              ...
              ${
                  presentes
                  ?`${presentes}/${grupo.membros.length} presentes`
                  :"Pendente"
              }
...
function renderizarModal(){
  ...
  ?`
      ...
      <img
          class="foto-principal foto-ampliavel"
          src="${Util.escapar(principal.fotoUrl)}"
          alt="">
  ...
  document.getElementById("conteudoModal").innerHTML=`
      ...
      <button
          id="salvarPresencas"
          class="botao-principal largura-total">
  ...
  const imagem=document.querySelector(".foto-ampliavel");
...
async function salvar(){
  ...
  try{
      ...
      for(const caixa of caixas){
          ...
          estado.convidados.find(
              p=>p.id===caixa.dataset.id
          );

          if(
              !pessoa||
              pessoa.checkin===caixa.checked
          )continue;
      ...
      estado.grupos=
      Util.agrupar(estado.convidados);
